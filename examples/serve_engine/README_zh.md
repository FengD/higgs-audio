...
我们还提供了一个独立的脚本`generate_from_text.py`，专门用于从文本生成语音：

```bash
python generate_from_text.py --text-file zh.txt --output-file output_custom.wav
```

## 参数说明

- `--text-file`: 输入文本文件的路径（UTF-8编码）
- `--output-file`: 输出音频文件的路径（默认为output_from_text.wav）
